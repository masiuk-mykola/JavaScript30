...
console.log("inventorsFrom1950", inventorsFrom1500);
// Array.prototype.map()
// 2. Give us an array of the inventors first and last names
const inventorsNames = inventors.map((item) => `${item.first} ${item.last}`);
console.log("inventorsNames", inventorsNames);

// Array.prototype.sort()
// 3. Sort the inventors by birthdate, oldest to youngest
const inventorsByYear = inventors.sort((a, b) => a.year - b.year);
console.log("inventorsByYear", inventorsByYear);

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?

const inventorsYears = inventors.reduce((total, item) => {
  return total + (item.passed - item.year);
}, 0);
console.log("inventorsYears", inventorsYears);

// 5. Sort the inventors by years lived
const inventorsByYearsLived = inventors.reduce((acc, item) => {
  acc.push(item.passed - item.year);
  return acc;
}, []);
...
// const dee = de.filter((item) => item.includes("de"));
// 7. sort Exercise
// Sort the people alphabetically by last name
const byLastName = people.sort();
console.log("byLastName", byLastName);
// 8. Reduce Exercise
